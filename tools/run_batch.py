...
def process_docx(mode, config, custom_input_dir=None):
    if mode == 'rps':
        input_dir = custom_input_dir or config.get('input_rps')
        output_dir = config.get('output_rps')
        script = 'tools/edit_rps.py'
        is_folder_output = False
    elif mode == 'kontrak':
        input_dir = custom_input_dir or config.get('input_kontrak')
        output_dir = config.get('output_kontrak')
        script = 'tools/edit_konkul.py'
        is_folder_output = False
    elif mode == 'sap':
        input_dir = custom_input_dir or config.get('input_sap')
        output_dir = config.get('output_sap')
        script = 'tools/edit_sap.py'
...
def process_pdf(mode, config, custom_output_dir=None):
    target = mode.split('-')[1] # rps, sap, atau kontrak
    output_dir = custom_output_dir or config.get(f'output_{target}')
    ...
    script = 'tools/docx_to_pdf_folder.py'
...
def main():
    ...
    parser.add_argument('-f', '--folder', help='Custom input folder (untuk mode DOCX) atau output folder (untuk mode PDF)')
    ...
    if args.mode == 'all':
        for m in ['rps', 'sap', 'kontrak']:
            process_docx(m, config, args.folder)
        for m in ['rps', 'sap', 'kontrak']:
            process_pdf(f"pdf-{m}", config, args.folder)
    elif args.mode.startswith('pdf-'):
        process_pdf(args.mode, config, args.folder)
    else:
        process_docx(args.mode, config, args.folder)
